        ...
        if (event.sourceId == '') {
          if (Utils.getUserRole() == "BOSS") {
            if (event.searchAgian) {
              if (event.storeId == "") {
                path = "/v1/product/transfer/list?sort=desc&page=1&limit=1000";
              } else {
                // path =
                // "/v1/product/transfer/list?page=${event.page}&sort=desc&limit=1000&q=${event.searchValue}";
              }
            } else {
              path =
                  '/v1/product/transfer/list?page=${event.page}&is_warehouse=1&limit=1000&sort=desc';
            }
          } else {
            if (event.searchAgian) {
              // path =
              ...
              path =
                  '/v1/product/transfer/list?limit=1000&store_id=${Utils.getStoreId()}&page=${event.page}&sort=desc';
            }
          }
        } else {
          path =
              '/v1/product/transfer/list?limit=1000&page=${event.page}&sort=desc&tid=${event.sourceId}';
        }
