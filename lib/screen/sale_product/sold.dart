...
class _SoldItemMainScreenState extends State<SoldItemMainScreen> with BaseStateMixin {
  ...
  StoreDetialModel defaultStoreModel = StoreDetialModel(name: "Агуулах", phone_number: "", created_at: DateTime.now().toString(), is_main: 1, id: -1);
  ...
  Future<void> _getProductData() async {
    ...
    try {
      DetialSaleProductModel res = await api.getSoldProduct(_page, date, dateTime, storeId, searchAgian);
      setState(() {
        listData = res;
        if (res.warehouse != null) {
          otpWarehouse = SaleListModel(
              ...
              total_price_sell: listData.warehouse!.total_price_sell);
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Expanded(
                    child: InkWell(
                        ...
                        child: Container(
                            decoration:
                                BoxDecoration(color: kWhite, boxShadow: [BoxShadow(color: Colors.grey.shade300, blurRadius: 3, offset: const Offset(2, 2))], borderRadius: BorderRadius.circular(10)),
                            child: SizedBox(
                                height: 40, child: Center(child: Text("Эхлэх огноо ${date.year}-${date.month}-${date.day}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500))))))),
                ...
                Expanded(
                    child: InkWell(
                        ...
                        child: Container(
                            ...
                            child: SizedBox(
                                height: 40,
                                child: Center(child: Text("Дуусах огноо ${dateTime.year}-${dateTime.month}-${dateTime.day}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500)))))))
              ])),
          Padding(
                    ...
                    return Padding(
                        ...
                        child: Container(
                            decoration: BoxDecoration(
                                color: listModel![index].total_cost != 0 ? Colors.green.shade300 : kWhite,
                            ...
                            child: ListTile(
                                onTap: () {
                                  if (listModel[index].total_price != 0) {
                                    Navigator.of(context).push(CupertinoPageRoute(builder: (context) => SoldItemScreen(begindate: date, endDate: dateTime, id: listModel[index].store_id!)));
                                  }
                                },
                                leading: Container(
                                    height: 80.0,
                                    ...
                                    child: ClipRRect(borderRadius: BorderRadius.circular(9), child: Image.asset("assets/images/saleProduct.jpg", fit: BoxFit.cover))),
                                title: Text(
                                ...
                                subtitle: Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Text("Он сар: ${listData.date}", style: const TextStyle(fontSize: 12.0, fontWeight: FontWeight.normal)),
                                  Utils.getUserRole() == "BOSS"
                                      ? Text("Авсан үнэ: ${format.format(listModel[index].total_cost ?? 0)}₮", style: const TextStyle(fontSize: 12.0, fontWeight: FontWeight.normal))
                                      : const SizedBox.shrink(),
                                  Text('Зарах үнэ: ${format.format(listModel[index].total_price ?? 0)}₮', style: const TextStyle(fontSize: 11.0, fontWeight: FontWeight.normal)),
                                  Text('Зарсан үнэ: ${format.format(listModel[index].total_price_sell ?? 0)}₮', style: const TextStyle(fontSize: 11.0, fontWeight: FontWeight.normal)),
                                  Text('Төлөв: ${listModel[index].total_cost != 0 ? "Борлуулалттай" : "Борлуулалтгүй"}', style: const TextStyle(fontSize: 11.0, fontWeight: FontWeight.normal))
                                ]))));
                  })),
          Padding(
                      ...
                      child: SizedBox(
                          ...
                          child: Padding(
                              ...
                              child: Row(children: [
                                ...
                                Expanded(
                                    child:
                                        Text("Нийт үнийн дүн: ${show ? listData.total!.price_sell : ""}₮", textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)))
                              ]))))))
        ]));
  }
}
