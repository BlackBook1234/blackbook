    ...
    return Consumer<ProductProvider>(builder: (context, provider, child) {
      return Scaffold(
          appBar: AppBar(
              ...
              title: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.shopping_cart, color: kPrimaryColor), SizedBox(width: 5), Text("Сагс", style: TextStyle(color: kWhite, fontSize: 20, fontWeight: FontWeight.bold))]),
                          ...
                          child: Container(
                              ...
                              child: Theme(
                                  ...
                                  child: ExpansionTile(
                                      trailing: InkWell(
                                          onTap: () {
                                            provider.removeOrderItem(provider.lstDraft[index]);
                                          },
                                          child: const Icon(Icons.delete_outline_rounded, size: 30, color: kPrimaryColor)),
                                      shape: const Border(),
                                      ...
                                      leading: Container(
                                          ...
                                          child: ClipRRect(borderRadius: BorderRadius.circular(9), child: provider.lstDraft[index].photo == null ? Image.asset("assets/images/saleProduct.jpg", fit: BoxFit.cover) : Image.network(provider.lstDraft[index].photo!, fit: BoxFit.cover))),
                                      ...
                                      subtitle: Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        Text("Барааны код: ${provider.lstDraft[index].code}", style: const TextStyle(fontSize: 12.0, fontWeight: FontWeight.normal)),
                                        Utils.getUserRole() == "BOSS" ? Text('Авсан үнэ: ${provider.lstDraft[index].sizes!.first.cost}₮', style: const TextStyle(fontSize: 11.0, fontWeight: FontWeight.normal)) : const SizedBox.shrink(),
                                        Text('Зарах үнэ: ${provider.lstDraft[index].sizes!.first.price}₮', style: const TextStyle(fontSize: 11.0, fontWeight: FontWeight.normal))
                                      ]),
                                      ...
                                      children: [
                                        ...
                                        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                                          Text(getSizesString(provider.lstDraft[index].sizes), style: const TextStyle(fontSize: 11.0, fontWeight: FontWeight.normal)),
                                          Text(getWareString(provider.lstDraft[index].sizes), style: const TextStyle(fontSize: 11.0, fontWeight: FontWeight.normal))
                                        ])
                                      ]))));
                    })),
            Padding(
              ...
              child: BlackBookButton(
                ...
                onPressed: () {
                  showModalBottomSheet(
                      ...
                      builder: (context) {
                        return FractionallySizedBox(heightFactor: 0.7, child: ChooseStoreBottomSheetsWidget(data: provider.lstDraft, title: "Дэлгүүр сонгох"));
                      });
                },
