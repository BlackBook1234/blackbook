...
class NewDialog extends StatefulWidget {
  const NewDialog({super.key, required this.typeStore, required this.chosenStore, required this.categories, required this.userRole, required this.chosenType});
...
class _NewDialogState extends State<NewDialog> with BaseStateMixin {
  ...
  _getSummery() async {
    ...
    try {
      setState(() {
        for (CategoriesModel data in widget.categories) {
          if (widget.chosenType == data.name) {
            if (widget.chosenType == "Бүх төрөл") {
              chosenValue = "";
            } else {
              chosenValue = data.id.toString();
            }
      ...
      SummeryDetial res = await api.getSummery(chosenValue);
      setState(() {
        response = res;
        _runApi = false;
      });
    ...
    return Dialog(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Padding(padding: const EdgeInsets.symmetric(vertical: 20, horizontal: 20), child: SizedBox(height: 40, width: 40, child: ClipRRect(borderRadius: BorderRadius.circular(9), child: Image.asset("assets/images/mainLogo.png", fit: BoxFit.cover)))),
                ...
                children: [
                  Expanded(
                    child: Container(
                      width: 80,
                      height: 38,
                      decoration: BoxDecoration(color: kWhite, boxShadow: [BoxShadow(color: Colors.grey.shade400, blurRadius: 9, offset: const Offset(0, 0))], borderRadius: BorderRadius.circular(10)),
                      child: Padding(
                        ...
                        child: DropdownButton<String>(
                          ...
                          items: widget.typeStore.map<DropdownMenuItem<String>>((String value) {
                        ...
                        : SizedBox(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                if (chosenStore == response!.stores![index].name) {
                                  return Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Center(
