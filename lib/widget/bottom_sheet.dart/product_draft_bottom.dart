...
class _BottomSheetsWidgetState extends State<ProductBottomSheetsWidget> with BaseStateMixin {
  ...
  _addDataInDraft(List<ProductInDetialModel> data) {
    ...
    for (ProductInDetialModel otpList in data) {
      if (otpList.ware_stock != 0) {
        ProductInDetialModel inData = ProductInDetialModel(cost: data.first.cost, price: otpList.price, stock: otpList.ware_stock, id: otpList.id, type: otpList.type);
        setState(() {
          listData.add(inData);
          inData = inNullData;
        });
    ...
    if (listData.isEmpty) {
      ...
      Provider.of<ProductProvider>(GlobalKeys.navigatorKey.currentContext!, listen: false).setProductItemsData(draftData);
      setState(() {
